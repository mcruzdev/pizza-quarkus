...
class PizzaResourceTest {
  ...
  void should_place_order_correctly() {
    // given
    PizzaResource.Order order =
        new PizzaResource.Order(
  ...
  @Test
  void should_emit_message_to_socket_client_correctly()
  ...
  @Test
  void should_receive_dapr_events_correctly()
      throws DeploymentException, IOException, InterruptedException {
    try (Session session =
        ContainerProvider.getWebSocketContainer().connectToServer(Client.class, uri)) {
      daprClient.publishEvent(
          "pubsub",
          "topic",
          new PizzaResource.Event(
              PizzaResource.EventType.ORDER_PLACED,
              new PizzaResource.Order(
                  new PizzaResource.Customer("John Doe", "john.doe@email.com"),
                  List.of(new PizzaResource.OrderItem(PizzaResource.PizzaType.pepperoni, 1))),
              "service",
  ...
  void should_call_delivery_service_when_the_order_is_ready()
    ...
    try (Session session =
      ...
      daprClient.publishEvent(
          ...
          "topic",
          new PizzaResource.Event(
              PizzaResource.EventType.ORDER_READY,
              new PizzaResource.Order(
                  new PizzaResource.Customer("John Doe", "john.doe@email.com"),
                  List.of(new PizzaResource.OrderItem(PizzaResource.PizzaType.pepperoni, 1))),
              "service",
